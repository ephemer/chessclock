...
Meteor.methods({
  ...
  "game/switchPlayer": function (_id) {
    ...
    newData.lastAction = Meteor.isClient ? TimeSync.serverTime() : new Date();
...
function incrementCurrentPlayerTime(game, newData) {
  ...
  var now = Meteor.isClient ? moment(TimeSync.serverTime()) : moment();
  var timeSincelastAction = now.diff(game.lastAction);
